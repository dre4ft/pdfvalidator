...
// Coverage: JavaScript PDF exploits, XFA, OpenAction, SubmitForm, URI actions
...
rule pdf_malicious_structural
{
    ...
    strings:
        // PDF header marker
        $pdf_magic = "%PDF-"
        // Automatic action on open
        $openaction = "/OpenAction"
        // /AA (additional actions)
        $additional_action = "/AA"
        // AcroForm embedded form presence
        $acroform = "/AcroForm"
        // XFA form container (often exploitable via JS)
        $xfa = "/XFA"

    condition:
        $pdf_magic
        and any of ($openaction, $additional_action, $acroform, $xfa)
}

rule pdf_js_action_exploit
{
    meta:
        description = "Detect JavaScript actions within PDF that could be exploited (e.g. XFA, exportXFAData)"
        weight = 50

    strings:
        // JavaScript use in PDF action dictionaries
        $js_s_action = "/S /JavaScript"
        // JavaScript inside content
        $js_inside = "JavaScript("
        // XFA often used alongside PDF JS exploits
        $xfa_inline = "/XFA"
        // Common JS event functions often used in exploitation
        $js_event1 = "this.exportXFAData" nocase ascii
        $js_event2 = "app.openDoc" nocase ascii
        $js_alert  = "app.alert" nocase ascii

    condition:
        // Must be PDF and contain suspect JS
        uint16(0) == 0x2550 and
        any of ($js_s_action, $js_inside) and
        any of ($xfa_inline, $js_event1, $js_event2, $js_alert)
}
...
{
    meta:
        description = "Detect /SubmitForm with URL destination, often used for data exfiltration"
        weight = 45
...
{
    meta:
        description = "Detect PDF Actions pointing to remote URI (potential phishing / remote content exploitation)"
        weight = 35
...
{
    meta:
        description = "Detect embedded streams that may contain obfuscated JavaScript or shell code"
        weight = 30

    strings:
        $stream = "stream" nocase
        // Suspicious patterns common in obfuscated PDFs
...
{
    meta:
        description = "Detect suspicious PDF metadata fields often abused for social engineering"
        weight = 20
    ...
    condition:
        // Metadata alone is low priority but worth flagging
