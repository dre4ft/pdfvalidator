...
{
    meta: 
        author = "Glenn Edwards (@hiddenillusion) - adapted"
        version = "0.3"
        weight = 3
        description = "Detect PDF objects using multiple stream filters (possible obfuscation)"
    ...
    condition:
        ...
        $filter and
        (
            (#asciihex + #lzw + #ascii85 + #flate + #runlength) >= 2
        )
}
...
rule invalid_xref_numbers : PDF
{
        ...
        strings:
                $magic = { 25 50 44 46 }
                $reg0 = /xref\r?\n?.*\r?\n?.*65535\sf/
                $reg1 = /endstream.*endobj/
				$reg2 = /endobj.*startxref/
        condition:
                $magic at 0 and not $reg0 and not $reg1 and not $reg2
}
